import {
  ConversionOptions,
  ConversionResult,
  VideoInput,
  Environment
} from './core/types';
import { DEFAULT_CONFIG } from './config';

// Detect environment
function detectEnvironment(): Environment {
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    return 'browser';
  }
  return 'node';
}

/**
 * Main function to convert video to various formats
 */
export async function convertVideo(
  input: VideoInput,
  options: Partial<ConversionOptions>
): Promise<ConversionResult> {
  const env = detectEnvironment();

  // Merge options with defaults
  const fullOptions: ConversionOptions = {
    extractionMode: options.extractionMode || DEFAULT_CONFIG.extractionMode,
    outputFormat: options.outputFormat || DEFAULT_CONFIG.outputFormat,
    framesCount: options.framesCount,
    frameInterval: options.frameInterval,
    timeInterval: options.timeInterval,
    outputPath: options.outputPath,
    pdfLayout: options.pdfLayout || DEFAULT_CONFIG.pdfLayout,
    framesPerPage: options.framesPerPage || DEFAULT_CONFIG.framesPerPage,
    pageSize: options.pageSize || DEFAULT_CONFIG.pageSize,
    gifFps: options.gifFps || DEFAULT_CONFIG.gifFps,
    gifQuality: options.gifQuality || DEFAULT_CONFIG.gifQuality,
    gifRepeat: options.gifRepeat !== undefined ? options.gifRepeat : DEFAULT_CONFIG.gifRepeat,
    imageFormat: options.imageFormat || DEFAULT_CONFIG.imageFormat,
    imageQuality: options.imageQuality || DEFAULT_CONFIG.imageQuality,
    imagePrefix: options.imagePrefix || DEFAULT_CONFIG.imagePrefix,
    width: options.width,
    height: options.height,
    maintainAspectRatio: options.maintainAspectRatio !== false,
    compression: options.compression || DEFAULT_CONFIG.compression,
    onProgress: options.onProgress,
    onComplete: options.onComplete,
    onError: options.onError
  };

  try {
    let result: ConversionResult;

    if (env === 'node') {
      result = await processNodeEnvironment(input, fullOptions);
    } else {
      result = await processBrowserEnvironment(input, fullOptions);
    }

    if (fullOptions.onComplete) {
      fullOptions.onComplete(result);
    }

    return result;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    const result: ConversionResult = {
      success: false,
      error: errorMessage,
      metadata: {
        totalFrames: 0,
        extractedFrames: 0,
        duration: 0,
        dimensions: { width: 0, height: 0 }
      }
    };

    if (fullOptions.onError) {
      fullOptions.onError(error instanceof Error ? error : new Error(errorMessage));
    }

    return result;
  }
}

/**
 * Process video in Node.js environment
 */
async function processNodeEnvironment(
  input: VideoInput,
  options: ConversionOptions
): Promise<ConversionResult> {
  // Dynamic imports for Node.js-specific modules
  const { VideoProcessorNode } = await import('./core/VideoProcessorNode');
  const { PDFExporterNode } = await import('./exporters/PDFExporterNode');
  const { GIFExporterNode } = await import('./exporters/GIFExporterNode');
  const { ImageExporterNode } = await import('./exporters/ImageExporterNode');
  const nodeUtils = await import('./utils/node');

  // Create video processor
  const processor = new VideoProcessorNode(options);

  try {
    // Process video and extract frames
    const { frames, metadata } = await processor.process(input);

    let outputPath: string | undefined;
    let result: Buffer | { files: string[]; directory: string };

    // Export based on output format
    switch (options.outputFormat) {
      case 'pdf': {
        const exporter = new PDFExporterNode(options, metadata);
        const pdfBuffer = await exporter.export(frames as any);

        // Save to file if outputPath specified
        if (options.outputPath) {
          const pathModule = await import('path');
          outputPath = options.outputPath;
          if (nodeUtils.isDirectory(outputPath)) {
            outputPath = pathModule.join(outputPath, 'output.pdf');
          }
          await nodeUtils.writeBufferToFile(outputPath, pdfBuffer);
        }

        result = pdfBuffer;
        break;
      }

      case 'gif': {
        const exporter = new GIFExporterNode(options, metadata);
        const gifBuffer = await exporter.export(frames as any);

        // Save to file if outputPath specified
        if (options.outputPath) {
          const pathModule = await import('path');
          outputPath = options.outputPath;
          if (nodeUtils.isDirectory(outputPath)) {
            outputPath = pathModule.join(outputPath, 'output.gif');
          }
          await nodeUtils.writeBufferToFile(outputPath, gifBuffer);
        }

        result = gifBuffer;
        break;
      }

      case 'images': {
        const exporter = new ImageExporterNode(options, metadata);
        const imageResult = await exporter.export(frames as any);
        outputPath = imageResult.directory;
        result = imageResult;
        break;
      }

      default:
        throw new Error(`Unsupported output format: ${options.outputFormat}`);
    }

    // Cleanup
    await processor.cleanup();

    return {
      success: true,
      outputPath,
      metadata
    };
  } catch (error) {
    await processor.cleanup();
    throw error;
  }
}

/**
 * Process video in browser environment
 */
async function processBrowserEnvironment(
  input: VideoInput,
  options: ConversionOptions
): Promise<ConversionResult> {
  // Dynamic imports for browser-specific modules
  const { VideoProcessorBrowser } = await import('./core/VideoProcessorBrowser');
  const { PDFExporterBrowser } = await import('./exporters/PDFExporterBrowser');
  const { GIFExporterBrowser } = await import('./exporters/GIFExporterBrowser');
  const { ImageExporterBrowser } = await import('./exporters/ImageExporterBrowser');

  // Create video processor
  const processor = new VideoProcessorBrowser(options);

  try {
    // Process video and extract frames
    const { frames, metadata } = await processor.process(input);

    let blob: Blob | undefined;
    let files: File[] | undefined;

    // Export based on output format
    switch (options.outputFormat) {
      case 'pdf': {
        const exporter = new PDFExporterBrowser(options, metadata);
        blob = await exporter.export(frames as any);
        break;
      }

      case 'gif': {
        const exporter = new GIFExporterBrowser(options, metadata);
        blob = await exporter.export(frames as any);
        break;
      }

      case 'images': {
        const exporter = new ImageExporterBrowser(options, metadata);
        const imageResult = await exporter.export(frames as any);
        files = imageResult.files;
        break;
      }

      default:
        throw new Error(`Unsupported output format: ${options.outputFormat}`);
    }

    // Cleanup
    await processor.cleanup();

    return {
      success: true,
      blob,
      files,
      metadata
    };
  } catch (error) {
    await processor.cleanup();
    throw error;
  }
}

// Export types for users
export * from './core/types';
export { DEFAULT_CONFIG } from './config';

// Default export
export default convertVideo;

